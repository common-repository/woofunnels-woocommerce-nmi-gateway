...
class WooFunnel_WFNMIGWC {

	public static $version = XLNMI_BWF_VERSION;

	public static function register() {
		$configuration = array(
			'basename'    => plugin_basename( NMI_GATEWAY_WOOCOMMERCE_FILE ),
			'version'     => self::$version,
			'plugin_path' => dirname( NMI_GATEWAY_WOOCOMMERCE_FILE ),
			'class'       => __CLASS__,
		);
	...
	public static function load_files() {
		...
		$get_global_path = dirname( NMI_GATEWAY_WOOCOMMERCE_FILE ) . '/woofunnels/';
		 ...
		 */
		require_once dirname( NMI_GATEWAY_WOOCOMMERCE_FILE ) . '/woofunnels/' . 'includes/class-woofunnels-dashboard-loader.php';

		if ( self::$version === BWF_VERSION ) {
			do_action( 'woofunnels_loaded', $get_global_path );
		} else if ( defined( 'BWF_DEV' ) && true === BWF_DEV ) {
