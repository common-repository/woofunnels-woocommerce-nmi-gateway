	...
	class SV_WC_Payment_Gateway_Integration_Subscriptions extends SV_WC_Payment_Gateway_Integration {
		...
		public function process_change_payment( $result, $order_id, $gateway ) {
			...
			try {
				...
				$this->get_gateway()->add_debug_message( $note, 'error' );
		...
		public function update_failing_payment_method( $subscription, $renewal_order ) {
			...
			$payment_token = $this->get_gateway()->get_order_meta( SV_WC_Order_Compatibility::get_prop( $renewal_order, 'id' ), 'payment_token' );

			$this->get_gateway()->update_order_meta( $subscription, 'payment_token', $payment_token );
			$this->get_gateway()->update_order_meta( $subscription, 'customer_id', $payment_token );
		}
