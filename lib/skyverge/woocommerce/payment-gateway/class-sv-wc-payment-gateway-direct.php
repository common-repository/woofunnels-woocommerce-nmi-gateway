...
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginFramework\\v5_2_1\\SV_WC_Payment_Gateway_Direct' ) ) :
	...
	abstract class SV_WC_Payment_Gateway_Direct extends SV_WC_Payment_Gateway {
		...
		public function process_payment( $order_id ) {
			...
			if ( is_array( $result = apply_filters( 'wc_payment_gateway_' . $this->get_id() . '_process_payment', true, $order_id, $this ) ) ) {
				\NMI_Gateway_Woocommerce_Logger::log( "XL NMI order id: $order_id, Process payment in function " . __FUNCTION__ . " at Line: " . __LINE__ . " result: " . print_r( $result, true ) );
			...
			try {
				...
				if ( $this->supports_tokenization() && 0 !== $order->get_user_id() && $this->get_payment_tokens_handler()->should_tokenize() && ( '0.00' === $order->payment_total || $this->tokenize_before_sale() ) ) {
				...
				\NMI_Gateway_Woocommerce_Logger::log( "Exception in function: " . __FUNCTION__ . " at Line: " . __LINE__ . " For order: $order_id as: " . print_r( $e->getMessage(), true ) );
		...
		protected function do_credit_card_transaction( $order, $response = null ) {
			...
			if ( $response->transaction_approved() ) {
				...
				$message = sprintf( /* translators: Placeholders: %1$s - payment method title, %2$s - environment ("Test"), %3$s - transaction type (authorization/charge), %4$s - card type (mastercard, visa, ...), %5$s - last four digits of the card */ esc_html__( '%1$s %2$s %3$s Approved: %4$s ending in %5$s', 'woocommerce-plugin-framework' ), $this->get_method_title(), $this->is_test_environment() ? esc_html_x( 'Test', 'noun, software environment', 'woocommerce-plugin-framework' ) : '', $this->perform_credit_card_authorization( $order ) ? esc_html_x( 'Authorization', 'credit card transaction type', 'woocommerce-plugin-framework' ) : esc_html_x( 'Charge', 'noun, credit card transaction type', 'woocommerce-plugin-framework' ), SV_WC_Payment_Gateway_Helper::payment_type_to_name( $card_type ), $last_four );
		...
		protected function do_transaction( $order ) {
			...
			if ( $response->transaction_approved() || $response->transaction_held() ) {
				...
				if ( $this->supports_tokenization() && 0 != $order->get_user_id() && $this->get_payment_tokens_handler()->should_tokenize() && ( $order->payment_total > 0 && ( $this->tokenize_with_sale() || $this->tokenize_after_sale() ) ) ) {
					...
					try {
						...
						if ( ! $response->transaction_held() && ! ( $this->supports( self::FEATURE_CREDIT_CARD_AUTHORIZATION ) && $this->perform_credit_card_authorization( $order ) ) ) {
							...
							$message = sprintf( /* translators: Placeholders: %s - failure message */ esc_html__( 'Tokenization Request Failed: %s', 'woocommerce-plugin-framework' ), $e->getMessage() );
							...
							// transaction has already been successful, but we've encountered an issue with the post-tokenization, add an order note to that effect and continue on
							$message = sprintf( /* translators: Placeholders: %1$s - payment method title, %2$s - failure message */ esc_html__( '%1$s Tokenization Request Failed: %2$s', 'woocommerce-plugin-framework' ), $this->get_method_title(), $e->getMessage() );
		...
		protected function do_add_payment_method_transaction( \WC_Order $order ) {
			...
			if ( $response->transaction_approved() ) {
				...
				if ( $this->is_credit_card_gateway() ) {
					...
					$message = sprintf( esc_html__( 'Nice! New payment method added: %1$s ending in %2$s (expires %3$s)', 'woocommerce-plugin-framework' ), $token->get_type_full(), $token->get_last_four(), $token->get_exp_date() );
					...
					$message = sprintf( esc_html__( 'Nice! New payment method added: %1$s account ending in %2$s', 'woocommerce-plugin-framework' ), $token->get_account_type(), $token->get_last_four() );
