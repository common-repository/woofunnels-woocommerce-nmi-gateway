...
class XLNMI_Upsell {
	...
	protected $plugin_path = NMI_GATEWAY_WOOCOMMERCE_FILE;

	/**
	 * construct
	 */
	public function __construct() {
		$this->name = 'XL WooCommerce NMI Gateway';
		$this->year = date( 'y' );
		if ( 1 === absint( date( 'n' ) ) ) {
...
//		add_action( 'admin_notices', array( $this, 'xl_halloween_sale_notice' ), 10 );
	...
	public function notice_enqueue_scripts() {
		wp_enqueue_style( 'xlnmi-notices-css', plugin_dir_url( $this->plugin_path ) . 'admin/assets/css/xlnmi-xl-notice.css', array(), NMI_GATEWAY_WOOCOMMERCE_VERSION );
	...
	protected function autonami_notice_html() {
		...
		$sub_heading       = 'Use Autonami to set up cart abandonment, post-purchase, win-back campaigns & more. Up-level your email and SMS marketing game.';
